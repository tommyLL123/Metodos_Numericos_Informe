...
El fenómeno de estudio se modela como un flujo continuo descrito por la Ecuación Diferencial Ordinaria (EDO):
\begin{equation}
    \frac{dx_t}{dt} = v(x_t, t), \qquad x_0 \sim \pi_0, \qquad t \in [0,1],
\end{equation}
donde $x_t \in \mathbb{R}^2$ representa la posición de una partícula en el instante $t$, $\pi_0$ es una distribución inicial simple (una distribución Normal multivariada estándar $\mathcal{N}(0, I)$) y $v(x_t, t)$ es un campo vectorial de velocidad que transporta las muestras hacia una distribución objetivo bidimensional $\pi_1$ al tiempo $t=1$. El punto parte de una posición inicial aleatoria en $t=0$ y la simulación corre hasta $t=1$, momento en el cual se espera que haya alcanzado la distribución objetivo.

Se consideran dos formas de definir el campo $v$: (a) un campo vectorial recto, construido mediante interpolación lineal $x_t = t x_1 + (1-t) x_0$ entre pares de puntos muestreados de $\pi_0$ y $\pi_1$, cuya velocidad resulta constante e igual a $v = x_1 - x_0$; y (b) un campo vectorial curvo de referencia, que introduce no linealidad en la trayectoria. Esta idea de emplear una velocidad constante para trazar trayectorias rectas corresponde al principio central propuesto por \citet{liu2022flow} en Rectified Flow y por \citet{lipman2022flow} en Flow Matching. Cabe notar que, en ambos trabajos, el campo de velocidad se obtiene habitualmente entrenando una red neuronal $v_\theta(x,t)$ mediante regresión condicional; en el presente estudio se opta por campos definidos analíticamente en lugar de entrenar dicha red, con el fin de aislar el efecto del método de integración numérica sin introducir la variabilidad propia del proceso de aprendizaje, tal como se detalla en los Supuestos. Como distribuciones objetivo $\pi_1$ se emplean conjuntos de datos sintéticos bidimensionales y reproducibles: una mezcla de gaussianas y el conjunto conocido como \textit{two moons}. Se usan ambas versiones del campo $v$ a propósito, dado que el objetivo es comparar si la forma del camino (recto frente a curvo) afecta el resultado al cambiar el método de integración.

\subsection{Supuestos y Simplificaciones}
Se adoptan las siguientes simplificaciones respecto al problema general abordado en la literatura, con el fin de enfocar el análisis en el desempeño del integrador numérico:

\begin{itemize}
    \item \textbf{Dimensionalidad reducida:} Se trabaja en dimensión $d=2$, en lugar de las imágenes de alta dimensión reportadas en los artículos de referencia. Esto permite visualizar directamente las trayectorias individuales y las distribuciones resultantes, además de reducir la carga computacional necesaria para evaluar múltiples métodos numéricos.
    \item \textbf{Campos vectoriales analíticos:} Se emplean campos de velocidad definidos mediante una fórmula matemática conocida, en lugar de entrenar una red neuronal como en el objetivo de regresión condicional de \citet{lipman2022flow}. Esta simplificación es clave: al usar una fórmula fija, se conoce con exactitud la trayectoria correcta, de modo que cualquier error observado en la distribución final se puede atribuir de forma predominante al método de integración numérica y no a un posible error de aprendizaje.
    \item \textbf{Suavidad del campo vectorial:} Se asume que los campos vectoriales considerados son suficientemente suaves, es decir, sin saltos ni cambios bruscos. Esto garantiza que la ecuación diferencial tenga una solución única en cada punto y evita que dos trayectorias distintas se crucen en el mismo lugar y momento, lo que haría ambigua la dirección del movimiento.
    \item \textbf{Ventaja de las trayectorias rectas:} Se asume que la rectitud de los flujos inducidos por Rectified Flow y Flow Matching reduce la curvatura del campo vectorial y, en consecuencia, el error de truncamiento local de los métodos numéricos, permitiendo el uso de tamaños de paso más grandes sin degradar significativamente la calidad de la solución.
\end{itemize}

\subsection{Métodos Numéricos Propuestos}
Para resolver la EDO planteada, la fase de generación requiere aproximar la solución desde $t=0$ hasta $t=1$. Se proponen tres esquemas de integración numérica que cubren distintos niveles de precisión y costo computacional.

Acorde a los lineamientos del curso, se emplean \textbf{dos métodos numéricos revisados en clase}:
\begin{enumerate}
    \item \textbf{Método de Euler (primer orden):} Se implementa como método base de comparación. En cada paso, se calcula la velocidad actual y se avanza en línea recta ese intervalo temporal. Es el método más económico computacionalmente, pero se espera que presente un mayor error de truncamiento frente a tamaños de paso grandes, lo que permite evaluar qué tan permisivas resultan las trayectorias rectas frente a un integrador de bajo orden.
    \item \textbf{Runge-Kutta de cuarto orden (RK4):} Se implementa como integrador de referencia de alta precisión. En cada paso evalúa la velocidad en cuatro puntos distintos y los combina para obtener una aproximación de mayor exactitud. Dado su mayor costo por iteración (cuatro evaluaciones del campo vectorial por paso), permite analizar el compromiso entre el costo computacional, medido como Número de Evaluaciones de la Función (NFE), y la precisión obtenida.
\end{enumerate}

Como \textbf{método numérico no revisado explícitamente en clase}, se propone:
\begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Runge-Kutta adaptativo (RK45, Fehlberg/Dormand-Prince):} A diferencia de los métodos de paso fijo, este integrador calcula simultáneamente una aproximación de cuarto y quinto orden para estimar el error de truncamiento local en cada iteración, y ajusta dinámicamente el tamaño de paso en función de dicha estimación. Se propone este método porque permite evaluar el costo computacional real necesario para alcanzar una tolerancia de error determinada: en las zonas donde la trayectoria sea recta, el paso se agranda automáticamente, optimizando el NFE sin comprometer la precisión de la distribución final obtenida.
\end{enumerate}

La elección de estos tres métodos responde directamente a los objetivos específicos (ii) y (iii) del estudio, pues permite construir curvas de error frente a costo computacional y analizar la sensibilidad del resultado final al tamaño de paso empleado, en línea con el análisis comparativo de integradores reportado por \citet{xiao2026solvers} para modelos de flow matching.
